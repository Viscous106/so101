...
run = pathlib.Path("runs") / f"{joint}_{time.strftime('%Y%m%d_%H%M%S')}"
...
meta = {"joint": joint, "P": P, "delta": delta, "n_pairs": n_pairs, "stair": STAIR, "started": time.strftime("%F %T"),
...
try:
    ...
    for s in STAIR:                                   # always arriving from below
        peak = b.move(joint, P + s); capture("stair", +1, P + s, peak)
    for k in range(n_pairs):
        b.move(joint, P + delta, settle=0.3); peak = b.move(joint, P); capture("repeat", -1, P, peak)   # arrive from above
        b.move(joint, P - delta, settle=0.3); peak = b.move(joint, P); capture("repeat", +1, P, peak)   # arrive from below
